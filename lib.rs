...
mod amm {
    ...
    // Using the same mathematical formula as used by Uniswap to determine the price of the assets (K = totalToken1 * totalToken2) 
    #[derive(Default)]
    #[ink(storage)]
    pub struct Amm {
    ...
    impl Amm {
        ...
        // Part 7. Provide
        // If the pool is initially empty then the equivalence rate is set as _amountToken1 : _amountToken2 and the user is issued 100 shares for it. Otherwise, it is checked whether the two amounts provided by the user have equivalent value or not.
